...
N = 6000
n = 0.00108
t, dt = np.linspace(0, N, N+1, retstep=True)
h=dt
g1 = 9.81 #m/s-2
...
def correc(kp, ki, kd, z_cible_tab, N_tab):
    ...
    for i in range(N):
        ...
        V1[i+1] = V
        if dV >= 0:
            E[i + 1] = E[i] + 1/(nvm * nreduc * nmoteur) * dt * -dV * Pression(y[i]) * 100000
        else:
            E[i + 1] = E[i] + (nem * nreduc * nmoteur) * dt * -dV * Pression(y[i]) * 100000
    print(E[-1] - E[0])
    plt.plot(t, -np.array(y), '--', label='Profondeur')
    plt.plot(t, v, label='vitesse')
    plt.xlabel("temps")
...
correc(1e-4, 1e-7, 1e-2, [100,300,0], [5/11*N,6/11*N])
